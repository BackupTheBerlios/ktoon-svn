    ...
    : QMainWindow( 0, "K-Toon", WDestructiveClose )
{
	...
	stream.readLine();  //The language variable. It doesn't matter here.
	recent1 = stream.readLine();
	recent2 = stream.readLine();
	recent3 = stream.readLine();
	recent4 = stream.readLine();
	recent5 = stream.readLine();
    }
    settings.close();
    ...
    //----------- Operations related with the Tool Bars --------------------
	...
	id_file_open_recent1 = open_recent -> insertItem( recent1 );
	id_file_open_recent2 = open_recent -> insertItem( recent2 );
	id_file_open_recent3 = open_recent -> insertItem( recent3 );
	id_file_open_recent4 = open_recent -> insertItem( recent4 );
	id_file_open_recent5 = open_recent -> insertItem( recent5 );
    ...
    connect( current_drawing_area, SIGNAL( colorGrabbed( Color * ) ), color_palette_dialog, SLOT( slotSetColor( Color * ) ) );
...
void Ktoon::loadLibrary( const QString &file_name, bool from_load )
{
    ...
    QDomDocument library_doc( Document::turnSlashesIntoUnderscores( file_name ) );
...
void Ktoon::loadPalette( const QString &file_name, bool from_load )
{
    ...
    QDomDocument palette_doc( Document::turnSlashesIntoUnderscores( file_name ) );
...
void Ktoon::loadBrushes( const QString &file_name, bool from_load )
{
    ...
    QDomDocument brushes_doc( Document::turnSlashesIntoUnderscores( file_name ) );
...
{
    QString fn = QFileDialog::getOpenFileName( "projects/", "Ktoon Project ( *.ktn )", this );
    if ( !fn.isEmpty() )
    {
...
void Ktoon::slotLoadDocument( const QString &in_file_name )
{
    ...
    QDomDocument xml_doc( Document::turnSlashesIntoUnderscores( in_file_name ) );
    if ( !xml_doc.setContent( &f ) )
    {
    ...
    QString palette_path = palette_path_tag.attribute( "PATH" );
    ...
    QString brushes_path = brushes_path_tag.attribute( "PATH" );
    ...
    QString library_path = library_path_tag.attribute( "PATH" );
    loadLibrary( Document::turnUnderscoresIntoSlashes( library_path ), true );
...
{
    QString fn = QFileDialog::getSaveFileName( "projects/", "Ktoon Project ( *.ktn )", this );
    if ( !fn.isEmpty() )
    {
	file_name = fn + ".ktn";
...
void Ktoon::slotAboutOpenGL()
{
    ...
    logo -> move( 144, 10 );
    logo -> resize( 112, 70 );
    QPixmap i_logo = QPixmap( "../ktoon/src/ima/ogllogo.png", "PNG" );
...
void Ktoon::slotStatusBarMessage( int mi )
{
    ...
    else if( id_file_open_recent1 == mi ) statusBar() -> message( tr( "Opens" ) + " " + open_recent -> text( id_file_open_recent1 ), 2000 );
    else if( id_file_open_recent2 == mi ) statusBar() -> message( tr( "Opens" ) + " " + open_recent -> text( id_file_open_recent2 ), 2000 );
    else if( id_file_open_recent3 == mi ) statusBar() -> message( tr( "Opens" ) + " " + open_recent -> text( id_file_open_recent3 ), 2000 );
    else if( id_file_open_recent4 == mi ) statusBar() -> message( tr( "Opens" ) + " " + open_recent -> text( id_file_open_recent4 ), 2000 );
    else if( id_file_open_recent5 == mi ) statusBar() -> message( tr( "Opens" ) + " " + open_recent -> text( id_file_open_recent5 ), 2000 );
...
void Ktoon::closeEvent( QCloseEvent *close_event )
{
	...
	else
	{
	    ...
	    {
		settings.close();
		settings.open( IO_ReadWrite );
		QTextStream read_write( &settings );
		read_write.readLine();  //The language variable. It doesn't matter here.
		settings.flush();
		for ( QStringList::Iterator it = recent_names.begin(); it != recent_names.end(); ++it )
		    read_write << *it << "\n";
	    }
...
{
    open_recent -> changeItem( id_file_open_recent1, recent_names[0] );
    open_recent -> changeItem( id_file_open_recent2, recent_names[1] );
    open_recent -> changeItem( id_file_open_recent3, recent_names[2] );
    open_recent -> changeItem( id_file_open_recent4, recent_names[3] );
    open_recent -> changeItem( id_file_open_recent5, recent_names[4] );
}
