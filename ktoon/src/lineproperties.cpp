...
LineProperties::LineProperties( QWidget *parent ) : QDialog( parent, "Line Properties", true )
{
    ...
    resize( 235, 180 );
    setMinimumSize( 235, 180 );
    setMaximumSize( 235, 180 );
    ...
    text_end = new QLabel( tr( "End Point" ) + QString( ":" ), this );
    text_end -> resize( 100, 20 );
    text_end -> move( text_origin -> x(), text_origin -> y() + text_origin -> height() + 5 );

    text_end_x = new QLabel( "X", this );
    text_end_x -> resize( 15, 20 );
    text_end_x -> move( text_end -> x() + text_end -> width() + 5, text_end -> y() );

    text_end_y = new QLabel( "Y", this );
    text_end_y -> resize( 15, 20 );
    text_end_y -> move( text_end_x -> x() + text_end_x -> width() + 45, text_end_x -> y() );

    text_pattern = new QLabel( tr( "Border Style" ) + QString( ":" ), this );
    text_pattern -> resize( 100, 20 );
    text_pattern -> move( text_end -> x(), text_end -> y() + text_end -> height() + 5 );
    ...
    value_end_x = new QLineEdit( this );
    value_end_x -> resize( 30, 20 );
    value_end_x -> move( text_end_x -> x() + text_end_x -> width() + 5, text_end_x -> y() );

    value_end_y = new QLineEdit( this );
    value_end_y -> resize( 30, 20 );
    value_end_y -> move( text_end_y -> x() + text_end_y -> width() + 5, text_end_y -> y() );
...
void LineProperties::loadFromGraphic( GLLine *line )
{
   ...
   value_end_y -> setText( QString::number( line -> endLine().y() ) );
   switch ( line -> stipplePattern() )
     {
        case 0xFFFF:
   ...
   value_factor -> setText( QString::number( line -> stippleFactor() ) );
   value_angle -> setText( QString::number( 0 ) );
}

//------------------- SLOTS -------------------------

void LineProperties::slotAccept()
{
   GLLine *line = ( GLLine * )drawing_area -> currentGraphic();
   line -> setOriginPoint( QPoint( ( value_origin_x -> text() ).toInt(), ( value_origin_y -> text() ).toInt() ) );
   line -> setEndLine( QPoint( ( value_end_x -> text() ).toInt(), ( value_end_y -> text() ).toInt() ) );
   switch ( value_pattern -> currentItem() )
       {
   	  case 0:
	  	line -> setStipplePattern( 0xFFFF );
		break;
	  case 1:
	  	line -> setStipplePattern( 0x5555 );
		break;
	  case 2:
	  	line -> setStipplePattern( 0x0707 );
		break;
	  case 3:
	  	line -> setStipplePattern( 0x1111 );
		break;
	  case 4:
	  	line -> setStipplePattern( 0x6F6F );
		break;
	  case 5:
	  	line -> setStipplePattern( 0x9F9F );
		break;
	  case 6:
	  	line -> setStipplePattern( 0x5F5F );
		break;
	  case 7:
	  	line -> setStipplePattern( 0x2727 );
		break;
	  default:
		break;

       }
   line -> setStippleFactor( ( value_factor -> text() ).toUShort() );
   line -> setRotationAngle( ( value_angle -> text() ).toFloat() );
   drawing_area -> updateGL();
   drawing_area -> modifyDocument( true );
   close( true );
}

void LineProperties::slotCancel()
{
    close( true );
}
