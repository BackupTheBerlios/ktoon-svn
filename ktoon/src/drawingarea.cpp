...
void DrawingArea::initializeGL()
{
    ...
    id_texture = loadTexture( "../ktoon/src/ima/arrow_selection.xpm" );
    id_spin = loadTexture( "../ktoon/src/ima/spin.xpm" );
   ...
   else
   {
    ...
    switch ( current_cursor )
	{
	 ...
	 case Tools::BRUSH:
	        {
		invertMatrix();
		current_graphic = new GLBrush( this, mapPointToMatrix( oldPosition() ), *current_outline_color,
...
void DrawingArea::keyPressEvent( QKeyEvent *key_event )
{
    ...
    if ( current_graphic )
         current_graphic -> keyPressEvent( key_event );
    updateGL();
}
...
void DrawingArea::slotAlignRight()
{
  ...
  else if( current_graphic )
    {
       modifyDocument( true );
       current_graphic -> calculateBottomRight();
       QPoint *point = new QPoint( current_graphic -> bottomRight() );
       int x = current_graphic -> getTranslate().x();
       current_graphic -> translateGraphic( -x, 0 );
       x = maxHorizontal();
       current_graphic -> translateGraphic( ( x - ( point -> x() ) ), 0 );
     }
}
  ...
  if( current_graphic )
    {
	 ...
	 current_graphic -> calculateTopLeft();
	 current_graphic -> calculateBottomRight();
	 QPoint *point_tl = new QPoint( current_graphic -> topLeft() );
	 QPoint *point_br = new QPoint( current_graphic -> bottomRight() );
         int y = current_graphic -> getTranslate().y();
         current_graphic -> translateGraphic( 0, -y );
	 y = maxVertical();
	 current_graphic -> translateGraphic( 0, -( point_tl -> y() ) );
	 current_graphic -> translateGraphic( 0, ( int ) ( ( y - ( point_br -> y() - point_tl -> y() ) ) / 2 ) );
    }
}
  ...
  if( current_graphic )
    {
         ...
         int x = current_graphic -> getTranslate().x();
         current_graphic -> translateGraphic( -x, 0 );
         current_graphic -> translateGraphic( -( point -> x() ), 0 );
    }
}
  ...
  if( current_graphic )
    {
	 ...
	 QPoint *point = new QPoint( current_graphic -> topLeft() );
         int y = current_graphic -> getTranslate().y();
         current_graphic -> translateGraphic( 0, -y );
	 current_graphic -> translateGraphic( 0, - ( point -> y() ) );
     }
}
...
void DrawingArea::slotCenterHorizontally()
{
  if( current_graphic )
	...
	QPoint *point_br = new QPoint( current_graphic -> bottomRight() );
        ...
        current_graphic -> translateGraphic( -x, 0 );
	x = maxHorizontal();
	current_graphic -> translateGraphic( -( point_tl -> x() ), 0 );
	current_graphic -> translateGraphic( ( int ) ( ( x - ( point_br -> x() - point_tl -> x() ) ) / 2 ), 0 );
    }
}
...
void DrawingArea::slotAlignBottom()
{
  if( current_graphic )
	...
	QPoint *point = new QPoint( current_graphic -> bottomRight() );
        ...
        current_graphic -> translateGraphic( 0, -y );
	y = maxVertical();
	current_graphic -> translateGraphic( 0, ( y - ( point -> y() ) ) );
     }
}
...
void DrawingArea::slotFlipHorizontally()
{
  if( current_graphic )
    {
   	modifyDocument( true );
   	current_graphic -> toggleHorizontalFlip();
    }
}
...
void DrawingArea::slotFlipVertically()
{
  if( current_graphic )
    {
        modifyDocument( true );
        current_graphic -> toggleVerticalFlip();
    }
}
...
void DrawingArea::slotRotateCW90()
{
  if( current_graphic )
    {
        modifyDocument( true );
        current_graphic -> setRotationAngle( 90.0 );
    }
}
...
void DrawingArea::slotRotate180()
{
  if( current_graphic )
    {
        modifyDocument( true );
        current_graphic -> setRotationAngle( 180.0 );
    }
}
   ...
   if( current_graphic )
    ...
    switch ( current_cursor )
	{
	 case Tools::NORMAL_SELECTION:
			...
			{
			 case GLGraphicComponent::GC_RECTANGLE:
			     ( ( GLRectangle * )( current_graphic ) ) -> setFillColor( *current_fill_color );
			     break;
			 case GLGraphicComponent::GC_ELLIPSE:
			     ( ( GLEllipse * )( current_graphic ) ) -> setFillColor( *current_fill_color );
			     break;
			 default:
			     break;
			}
...
void DrawingArea::slotChangeMinThicknessBrush()
{
   ...
   if( current_graphic )
	...
	{
	 case Tools::NORMAL_SELECTION:
		  current_graphic -> setWidthPoint( *current_brush );
                  break;
	 case Tools::CONTOUR_SELECTION:
                  break;
	 case Tools::BRUSH:
    ...
    if ( current_graphic )
	 ...
	 switch( current_graphic -> kindGraphic() )
		 ...
		 case GLGraphicComponent::GC_RECTANGLE:
			...
			break;
		 case GLGraphicComponent::GC_ELLIPSE:
		        color = new Color( ( ( GLEllipse * )( current_graphic ) ) -> fillColor().colorRed(),
