...
void PenProperties::slotAccept()
{
   ...
   switch ( value_pattern -> currentItem() )
       {
   	  case 0:
	  	pen -> setStipplePattern( 0xFFFF );
		break;
	  case 1:
	  	pen -> setStipplePattern( 0x5555 );
		break;
	  case 2:
	  	pen -> setStipplePattern( 0x0707 );
		break;
	  case 3:
	  	pen -> setStipplePattern( 0x1111 );
		break;
	  case 4:
	  	pen -> setStipplePattern( 0x6F6F );
		break;
	  case 5:
	  	pen -> setStipplePattern( 0x9F9F );
		break;
	  case 6:
	  	pen -> setStipplePattern( 0x5F5F );
		break;
	  case 7:
	  	pen -> setStipplePattern( 0x2727 );
		break;
	  default:
		break;

       }
   pen -> setStippleFactor( ( value_factor -> text() ).toUShort() );
   pen -> setRotationAngle( ( value_angle -> text() ).toFloat() );
   drawing_area -> updateGL();
   drawing_area -> modifyDocument( true );
   close( true );
}

void PenProperties::slotCancel()
{
    close( true );
}
