...
KTRequestFactory::KTRequestFactory()
	: QXmlDefaultHandler()
{
}
...
bool KTRequestFactory::startElement( const QString& , const QString& , const QString& qname, const QXmlAttributes& atts)
{
	if (!m_isParsing)
	{
		m_isParsing = true;
...
bool KTRequestFactory::endElement(const QString&, const QString& , const QString& qname)
{
	if ( qname == "request" )
	{
		m_isParsing = false;
...
bool KTRequestFactory::characters ( const QString & ch )
{
	if (m_readCharacters )
	{
		...
		if ( m_qname == "data" )
		{
			if ( ch == "true" )
			{
				m_requestData.data = true;
			}
			else if ( ch == "false")
			{
				m_requestData.data = false;
			}
			else
			{
...
bool KTRequestFactory::error ( const QXmlParseException & exception )
{
...
bool KTRequestFactory::fatalError ( const QXmlParseException & exception )
{
...
KTProjectRequest *KTRequestFactory::createRequest()
{
...
KTProjectRequest *KTRequestFactory::build(const QString &document)
{
