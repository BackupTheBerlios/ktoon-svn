...
DiSplitter::DiSplitter(Qt::Orientation orientation, QWidget *parent, const char *name)
...
// 		setOpaqueResize(true);
	...
	DiSplitter::~DiSplitter()
	{
	}

	void DiSplitter::addDock(int row, int col, QWidget *dock)
	{
	...
	void DiSplitter::appendSplitter()
	{
	...
	void DiSplitter::createSplitters(int index)
	{;
	...
	void DiSplitter::removeDock(int row, int col, bool alsoDelete)
	{
	...
	bool DiSplitter::isRowEmpty(int row)
	{
	...
	void DiSplitter::shiftWidgets(QSplitter *splitter, int row, int fromCol)
	{
	...
	int DiSplitter::numRows() const
	{
		return m_docks.count();
	}

	int DiSplitter::numCols(int row) const
	{
		if (row < numRows())
	...
	QPair<int, int> DiSplitter::indexOf(QWidget *dock)
	{
