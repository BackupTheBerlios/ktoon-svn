...
bool OutlineColor::isActive()
{
    return active;
}

QColor OutlineColor::currentColor()
{
    return current_color;
}

short int OutlineColor::getAlpha()
{
    return alpha;
}

void OutlineColor::setActive( bool state )
{
    active = state;
...
void OutlineColor::slotSetColor( const QColor &new_color )
{
    if ( active )
    {
...
void OutlineColor::slotChangeAlpha( int new_alpha )
{
    if ( active )
    {
        alpha = new_alpha;
...
void OutlineColor::drawContents( QPainter *painter )
{
    ...
    if ( active )
    {
        QBrush b( white, Qt::Dense5Pattern );
