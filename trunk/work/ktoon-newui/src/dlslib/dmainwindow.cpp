...
void DMainWindow::loadSettings()
{
    ...
    config->setGroup("UI");
    m_pOpenTabAfterCurrent = config->readBoolEntry("OpenNewTabAfterCurrent", true);
    m_pShowIconsOnTabs = config->readBoolEntry("ShowTabIcons", false);
#else
    QSettings config;
...
#else
        const QIconSet &icons = (pixmap && (pixmap->size().height() <= 16)) ? *(pixmap) : QIconSet((const char **)icon_xpm);
#endif
