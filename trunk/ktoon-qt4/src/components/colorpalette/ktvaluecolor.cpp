...
KTItemValueColor::KTItemValueColor(  QString text, QWidget *parent ) :QFrame(parent)
{
	KTINIT;
...
KTValueColor::KTValueColor(QWidget *parent)
	: QFrame(parent)
{
	KTINIT;
...
void KTValueColor::setupForm()
{
	...
	m_layout->addWidget(m_valueH, 1, 0,Qt::AlignTop);
	m_layout->addWidget(m_valueS, 1, 1,Qt::AlignTop);
	m_layout->addWidget(m_valueV, 1, 2,Qt::AlignTop);
	m_layout->addWidget(m_valueA, 2, 1,Qt::AlignTop);
}

void KTValueColor::setColor(QColor color)
{
// 	ktDebug( ) << "aki";
// 	emitColor = false;
