...
GLRenderCameraPreview::GLRenderCameraPreview( QWidget* parent, /*QPopupMenu *in_assigned_menu, int id_assigned_item, QToolButton *assig_tb_button, */QGLWidget *share, Qt::WFlags f )
...
void GLRenderCameraPreview::paintGL()
{
    ...
    Q3PtrList<KeyFrame> keyframes_to_display = KTStatus -> renderKeyframes();
    ...
    for ( k_it = keyframes_to_display.first(); k_it; k_it = keyframes_to_display.next() )
    {
 	...
 	glTranslatef( 0.0, 0.0, position );
        Q3PtrList<GLGraphicComponent> gl = k_it -> getDrawing() -> graphicComponents();
...
void GLRenderCameraPreview::processHits( GLint hits, GLuint buffer[] )
{
    ...
    Q3PtrList<KeyFrame> displayed_keyframes = KTStatus -> renderKeyframes();
...
Q3ValueList<QImage> GLRenderCameraPreview::imageList()
{
	Q3ValueList<QImage> images;
...
{
	Q3ValueList<QImage> imgs = imageList();
	
	ktDebug() << "Images: " << imgs.count() << endl;
...
{
	ktDebug() << "Saving image: " << filename << endl;
	QPixmap exp;
