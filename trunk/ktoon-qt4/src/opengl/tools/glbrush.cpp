...
void GLBrush::smoothnessBrush( int smooth )
{
	...
	try 
	{
		...
		for ( unsigned int i = 0; i < points.count() - 2; i +=2 )
		{
			...
			if ( ( m1 + m2 ) != 180 )
			{
// 				ktError() << "Append to aux p1" << endl;
				aux.append( p1 );
				QPoint *aux1 = new QPoint( ( int ) ( ( p1 -> x() + p2 -> x() ) / 2 ),
							( int ) ( ( p1 -> y() + p2 -> y() ) / 2 ) );
				try 
				{
// 					ktError() << "Append to aux aux1" << endl;
					aux.append( aux1 );
				}
				catch(...)
				{
				...
				try 
				{
// 					ktError() << "Append to aux aux2" << endl;
					aux.append( aux2 );
				}
				catch(...)
				{
			...
			{
// 				ktError() << "Append to aux p1 and p2" << endl;
				aux.append( p1 );
				aux.append( p2 );
			}
		}
...
#ifdef DEBUG_GLBRUSH
		ktDebug() << "Points size: " << points.count() << endl;
		ktDebug() << "Aux Size: " << aux.count() << endl;	
#endif
		if(points.count() < aux.count() )
		{
			...
			ktDebug() << "Size points after build: " << points.count() << endl;
			for( uint c_diff = points.count(); c_diff != aux.count(); ++c_diff )
			{
				if ( stupid > 300 )
				{
#ifdef DEBUG_GLBRUSH
					ktError() << "Oops! an error has ocurred!" << endl;
#endif
					//aux.clear();
					break;
				}
			...
			ktDebug() << "Size points before build: " << points.count() << endl;
		}
	} // outer try block
	catch(...)
	{
#ifdef DEBUG_GLBRUSH
		ktError() << "Catched!" << endl;
#endif
		if( !aux.isEmpty() )
		{
			Q3PtrListIterator<QPoint> iter( aux );
			do
			{
...
void GLBrush::setPoints( Q3PtrList<QPoint> _points )
{
   #if 1
   points = _points;
   #else
...
Q3PtrList<QPoint> GLBrush::getPoints() const
{
    return points;
}

Q3PtrList<int> GLBrush::pressureBrush() const
{
    return pressure;
}
