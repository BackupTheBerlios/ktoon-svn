...
void PencilProperties::loadFromGraphic( GLPencil *pencil )
{
   ...
   value_factor -> setText( QString::number( pencil -> stippleFactor() ) );
...
void PencilProperties::slotAccept()
{
   ...
   switch ( value_pattern -> currentItem() )
       {
   	  case 0:
	  	pencil -> setStipplePattern( 0xFFFF );
		break;
	  case 1:
	  	pencil -> setStipplePattern( 0x5555 );
		break;
	  case 2:
	  	pencil -> setStipplePattern( 0x0707 );
		break;
	  case 3:
	  	pencil -> setStipplePattern( 0x1111 );
		break;
	  case 4:
	  	pencil -> setStipplePattern( 0x6F6F );
		break;
	  case 5:
	  	pencil -> setStipplePattern( 0x9F9F );
		break;
	  case 6:
	  	pencil -> setStipplePattern( 0x5F5F );
		break;
	  case 7:
	  	pencil -> setStipplePattern( 0x2727 );
		break;
	  default:
		break;

       }
   pencil -> setStippleFactor( ( value_factor -> text() ).toUShort() );
   pencil -> setRotationAngle( ( value_angle -> text() ).toFloat() );
   drawing_area -> updateGL();
   drawing_area -> modifyDocument( true );
   close( true );
}

void PencilProperties::slotCancel()
{
    close( true );
}
