...
KTFontChooser::KTFontChooser(QWidget *parent) : QFrame(parent)
{
	...
	mainLayout->addLayout(KTFormFactory::makeLine(tr("Family"), m_families));
	
	m_fontStyle = new QComboBox;
	connect(m_fontStyle, SIGNAL(activated (int)), this, SLOT(emitFontChanged( int)));
	mainLayout->addLayout(KTFormFactory::makeLine(tr("Style"), m_fontStyle));
	
	m_fontSize = new QComboBox;
	connect(m_fontSize, SIGNAL(activated (int)), this, SLOT(emitFontChanged( int)));
	mainLayout->addLayout(KTFormFactory::makeLine(tr("Size"), m_fontSize));
	
	setCurrentFont(font());
}


KTFontChooser::~KTFontChooser()
{
}

void KTFontChooser::loadFontInfo(const QString &family)
{
	QFontDatabase fdb;
...
void KTFontChooser::emitFontChanged(int )
{
	emit fontChanged();
}

void KTFontChooser::setCurrentFont(const QFont &font)
{
	QFontDatabase fdb;
...
QFont KTFontChooser::font() const
{
	QFontDatabase fdb;
