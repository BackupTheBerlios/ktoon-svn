...
KTViewDocument::KTViewDocument(QWidget *parent , const char *name, Qt::WFlags f) : KTMdiWindow(parent, name, f)
{
...
{
	gridGroup = new Q3ActionGroup( parent(), tr("&Grid" ));
	gridGroup->setExclusive( true ); 
	
	Q3Action *a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/nogrid.xpm" ), tr( "&No Grid" ), QKeySequence(), gridGroup);
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotNoGrid()));
	a->setToggleAction ( true );
	a->setStatusTip(tr("Hides the grid" ));
	
	a = new Q3Action(QPixmap(KTOON_HOME+"/images/icons/grid12.xpm" ),  tr( "&12 Field Grid" ), QKeySequence(), gridGroup);
	a->setToggleAction ( true );
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotSeeGrid12()));
	a->setStatusTip(tr("Shows a 12 field grid" ));
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/grid16.xpm" ), tr( "&16 Field Grid" ), QKeySequence(), gridGroup);
	a->setToggleAction ( true );
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotSeeGrid16()));
	a->setStatusTip(tr("Shows a 16 field grid" ));
	a->setOn(true);
	
	m_aSubGrid = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/subgrid.xpm" ), tr( "&Subgrid" ), QKeySequence(), parent()),
	...
	m_aFrontBackGrid = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/front_back_grid.xpm"), tr( "Grid to Front/Back" ), QKeySequence(), this);
	...
	m_aLightTable = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/light_table.xpm" ), tr( "&Light Table" ), QKeySequence(), parent());
	...
	m_aClose = new Q3Action(QPixmap(KTOON_HOME+"/images/icons/close.xpm" ), tr( "Cl&ose" ), tr("Ctrl+Shift+W"), parent());
	connect(m_aClose, SIGNAL(activated()), this, SLOT(close()));
	m_aClose->setStatusTip(tr("Closes the active document"));
	
	viewPreviousGroup = new Q3ActionGroup( this );
	viewPreviousGroup->setExclusive( true );
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/no_previous.xpm" ), tr( "No Previous" ), Qt::Key_1, viewPreviousGroup);
	...
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/previous.xpm" ), tr( "Previous One" ), Qt::Key_2, viewPreviousGroup);
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotPreviousOnionSkin()));
	a->setStatusTip(tr("Shows the previous onion skin" ));
	a->setToggleAction ( true );
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/previous2.xpm" ), tr( "Previous Two" ), Qt::Key_3, viewPreviousGroup);
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotPrevious2OnionSkin()));
	a->setStatusTip(tr("Shows the previous 2 onion skins" ));
	a->setToggleAction ( true );
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/previous3.xpm" ), tr( "Previous Three" ), Qt::Key_4, viewPreviousGroup);
	a->setToggleAction ( true );
	...
	viewNextGroup = new Q3ActionGroup( this );
	viewNextGroup->setExclusive( true );
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/no_next.xpm" ), tr( "No Next" ), Qt::CTRL+Qt::Key_1, viewNextGroup);
	a->setToggleAction ( true );
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotNoNextOnionSkin()));
	a->setStatusTip(tr("Disables next onion skin visualization" ));
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/next.xpm" ), tr( "Next One" ), Qt::CTRL+Qt::Key_2, viewNextGroup);
	a->setToggleAction ( true );
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotNextOnionSkin()));
	a->setStatusTip(tr("Shows the next onion skin"));
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/next2.xpm" ), tr( "Next Two" ), Qt::CTRL+Qt::Key_3, viewNextGroup);
	a->setToggleAction(true );
	connect(a, SIGNAL(activated()), m_drawArea, SLOT(slotNext2OnionSkin()));
	a->setStatusTip(tr("Shows the next 2 onion skins"));
	
	a = new Q3Action( QPixmap(KTOON_HOME+"/images/icons/next3.xpm" ), tr( "Next Three" ), Qt::CTRL+Qt::Key_4, viewNextGroup);
	a->setToggleAction(true );
