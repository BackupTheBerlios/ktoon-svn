...
void Export::slotAccept()
{
    ...
    switch ( group -> selectedId() )
    {
        case 0:
	{
            QString fn = Q3FileDialog::getSaveFileName( KTOON_REPOSITORY + "/output/", "PNG sequence ( *.png )", this );
            if ( !fn.isEmpty() )
	    {
		    QString message = "";
...
//         	    message = tr( "Could not Export Animation" );
        ...
        case 1:
	{
            QString fn = Q3FileDialog::getSaveFileName( KTOON_REPOSITORY + "/output/", "Single PNG ( *.png )", this );
            if ( !fn.isEmpty() )
	    {
		QPixmap exp;
		fn = fn + ".png";
...
// 		exp.convertFromImage( k_toon -> renderCameraPreview() -> grabFrameBuffer() );
        ...
        case 2:
	{
            QString fn = Q3FileDialog::getSaveFileName( KTOON_REPOSITORY + "/output/", "Macromedia Flash ( *.swf )", this );
            if ( !fn.isEmpty() )
	    {
	...
	case 3:
	{
            QString fn = Q3FileDialog::getSaveFileName( KTOON_REPOSITORY + "/output/", "Scalable Vector Graphics ( *.svg )", this );
            if ( !fn.isEmpty() )
	    {
		QPixmap exp;
		fn = fn + ".svg";
