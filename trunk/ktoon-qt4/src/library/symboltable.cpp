...
SymbolTable::SymbolTable( QWidget *parent ) : Q3ListView( parent, "", Qt::WStyle_NoBorder )
{
    Q_CHECK_PTR( parent );
...
void SymbolTable::contentsMousePressEvent( QMouseEvent *mouse_event )
{
    ...
    Q3ListView::contentsMousePressEvent( mouse_event );
    mouse_event -> accept();
    dragging = true;
}
...
{
    Q_CHECK_PTR( mouse_event );
    Q3ListView::contentsMouseMoveEvent( mouse_event );
    if ( dragging )
    {
        mouse_event -> accept();
...
{
    Q_CHECK_PTR( mouse_event );
    Q3ListView::contentsMouseReleaseEvent( mouse_event );
    mouse_event -> accept();
    dragging = false;
