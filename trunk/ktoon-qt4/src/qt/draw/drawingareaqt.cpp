...
void DrawingAreaQt::contentsMousePressEvent( QMouseEvent *mouse_event )
{
	...
	{
	    Q3CanvasItemList touched_objects = _canvas -> collisions( mapped_point );
		...
		applying_bezier = false;
		drawing_pen = false;
	    	selection_rectangle = new Q3CanvasRectangle( _canvas );
        ...
        case Tools::BRUSH:
	{
	    ...
	    ( ( Q3CanvasPolygonalItem * )current_graphic ) -> setPen( pen );
        ...
        case Tools::PEN:
	{
	    ...
	    ( ( Q3CanvasLine * )current_graphic ) -> setPen( QPen( co_color, current_brush -> thicknessMinBrush() ) );
	    ...
	    ( ( Q3CanvasRectangle * )current_graphic ) -> setPen( QPen( co_color, current_brush -> thicknessMinBrush() ) );
	    ( ( Q3CanvasRectangle * )current_graphic ) -> setBrush( QBrush( cf_color ) );
	    ...
	    ( ( Q3CanvasEllipse * )current_graphic ) -> setPen( QPen( co_color, current_brush -> thicknessMinBrush() ) );
	    ( ( Q3CanvasEllipse * )current_graphic ) -> setBrush( QBrush( cf_color ) );
	    current_graphic -> move( mapped_point.x(), mapped_point.y() );
        ...
        case Tools::FILL:
	{
	    ...
	    {
	        Q3CanvasItem *filled = touched_objects.first();
	        if ( filled -> rtti() == QtRectangle::RTTI || filled -> rtti() == QtEllipse::RTTI )
		    ( ( Q3CanvasPolygonalItem * )filled ) -> setBrush( cf_color );
	    }
	    else
	    {
        ...
        case Tools::REMOVE_FILL:
	{
	    ...
	    if ( touched_objects.count() > 0 )
	    {
	        Q3CanvasItem *filled = touched_objects.first();
	        if ( filled -> rtti() == QtRectangle::RTTI || filled -> rtti() == QtEllipse::RTTI )
		    ( ( Q3CanvasPolygonalItem * )filled ) -> setBrush( Qt::NoBrush );
	    }
	}
	    break;
        case Tools::DROPPER:
	...
	{
	    zoom_rectangle = new Q3CanvasRectangle( _canvas );
...
void DrawingAreaQt::contentsMouseMoveEvent( QMouseEvent *mouse_event )
{
	...
	QPoint mapped_point = inverseWorldMatrix().map( mouse_event -> pos() );    
		...
		break;
            case Tools::HAND:
	    {
		Q3CanvasItemList items = _canvas -> allItems();
